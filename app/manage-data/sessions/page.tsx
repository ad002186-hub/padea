...
export default async function ManageSessionsPage() {
  ...
  const [{ data: sessionsData }, { data: exclusionsData }] = await Promise.all([
    supabaseAdmin
      .from("sessions")
      .select("id, day_of_week, dinner_time, manager_name, schools(name), caterers(name)"),
    supabaseAdmin
      .from("exclusions")
      .select("id, date, cancelled_year_levels, reason, sessions(day_of_week, schools(name))")
      .gte("date", daysAgo(30))
  ...
  const cancelledSessions = (exclusionsData ?? []).map((row: any) => ({
    id: row.id as string,
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              {sessions.length} session{sessions.length !== 1 ? "s" : ""} · {cancelledSessions.length} cancellation{cancelledSessions.length !== 1 ? "s" : ""} in the last 30 days.
            </p>
          </div>
          <Link
            href="/exclusions/new"
