...
export default async function ManageAbsencesPage() {
  ...
  const [{ data, error }, { data: schools }] = await Promise.all([
    supabaseAdmin
      .from("absences")
    ...
    <div className="p-8 max-w-5xl mx-auto">
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900 dark:text-white">Absences</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              {absences.length} record{absences.length !== 1 ? "s" : ""} found.
              {error && <span className="ml-2 text-[#ef4444]">Error loading data.</span>}
            </p>
          </div>
          <Link href="/absences/new" className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#7c3aed] text-white text-sm font-semibold hover:bg-[#6d28d9] transition-colors">
      ...
      <AbsencesTable absences={absences} schools={(schools ?? []).map(s => s.name)} />
    </div>
  );
}
